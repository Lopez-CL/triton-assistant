...
**/
export function getLastWords(text: string, maxLength: number): string {
	// TODO: Implement this function!
	if(text.length <= maxLength+1) return text;
	const words = text.split(" ");
	let result = '';
	for(let i = words.length - 1; i >= 0; i--){
		if((words[i] + ' ' + result).length >maxLength+1) break;
		console.log((words[i] + ' ' + result).length)
...
export function extractLinkedInPosts(csvContent: string): LinkedInPost[] {
	...
	const viewsIndex = header.indexOf('numViews');
	const sharesIndex = header.indexOf('numShares');
	...
	for (let i = 1; i < records.length; i++) {
		...
		if (
			fields.length <= Math.max(textIndex, typeIndex, dateIndex, urlIndex, likesIndex, viewsIndex, sharesIndex)
		) {
			continue;
		}

		const post: LinkedInPost = {
			contentType: fields[typeIndex],
			text: fields[textIndex],
			date: fields[dateIndex],
			url: fields[urlIndex],
			likes: parseInt(fields[likesIndex]) || 0,
			views: parseInt(fields[viewsIndex]) || 0,
			shares: parseInt(fields[sharesIndex]) || 0,
		};
